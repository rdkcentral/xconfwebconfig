...
func TestAcquireLock(t *testing.T) {
	...
	// Acquire a new lock
	err := dbClient.AcquireLock(db.GetDefaultTenantId(), lockName, lockUser1, ttlSeconds)
	assert.NilError(t, err)

	// Verify lock info
	lockInfo, err := dbClient.GetLockInfo(db.GetDefaultTenantId(), lockName)
	assert.NilError(t, err)
	assert.Equal(t, lockInfo["locked_by"], lockUser1)

	// Fail to acquire an existing lock
	err = dbClient.AcquireLock(db.GetDefaultTenantId(), lockName, lockUser2, ttlSeconds)
	assert.ErrorContains(t, err, "failed to acquire lock")

	// Fail to release a lock not owned by the user
	err = dbClient.ReleaseLock(db.GetDefaultTenantId(), lockName, lockUser2)
	assert.ErrorContains(t, err, "failed to release lock")

	// Release the lock
	err = dbClient.ReleaseLock(db.GetDefaultTenantId(), lockName, lockUser1)
	assert.NilError(t, err)
	...
	shortTtl := 1
	err = dbClient.AcquireLock(db.GetDefaultTenantId(), lockName, lockUser1, shortTtl)
	assert.NilError(t, err)
...
func TestLockTable(t *testing.T) {
	...
	// Acquire a new lock
	err := modelTableLock.Lock(db.GetDefaultTenantId(), lockUser1)
	assert.NilError(t, err)

	// Fail to acquire an existing lock
	err = modelTableLock.Lock(db.GetDefaultTenantId(), lockUser2)
	assert.ErrorContains(t, err, "failed to acquire lock")

	// Fail to release a lock not owned by the user
	err = modelTableLock.Unlock(db.GetDefaultTenantId(), lockUser2)
	assert.ErrorContains(t, err, "failed to release lock")

	// Release the lock
	err = modelTableLock.Unlock(db.GetDefaultTenantId(), lockUser1)
	assert.NilError(t, err)
	...
	err = envLockTable.Lock(db.GetDefaultTenantId(), lockUser1)
	assert.NilError(t, err)

	time.Sleep(1 * time.Second)

	// Acquire the now-expired lock
	err = envLockTable.Lock(db.GetDefaultTenantId(), lockUser2)
	assert.NilError(t, err)
...
func TestTableRowLock(t *testing.T) {
	...
	err := modelTableLock.LockRow(db.GetDefaultTenantId(), lockUser1, "key1")
	assert.NilError(t, err)

	err = modelTableLock.LockRow(db.GetDefaultTenantId(), lockUser2, "key2")
	assert.NilError(t, err)

	// Fail to acquire an existing lock on the same row
	err = modelTableLock.LockRow(db.GetDefaultTenantId(), lockUser1, "key2")
	assert.ErrorContains(t, err, "failed to acquire lock")

	// Release lock for key2
	err = modelTableLock.UnlockRow(db.GetDefaultTenantId(), lockUser2, "key2")
	assert.NilError(t, err)
	...
	// Acquire the now-expired lock
	err = modelTableLock.LockRow(db.GetDefaultTenantId(), lockUser2, "key1")
	assert.NilError(t, err)
}
