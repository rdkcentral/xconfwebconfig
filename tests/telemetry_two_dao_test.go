...
func TestGenericNamedListDaoForMacs(t *testing.T) {
	...
	assert.NilError(t, err)
	err = db.GetCompressingDataDao().SetOne(db.DEFAULT_TENANT_ID, db.TABLE_GENERIC_NS_LIST, sourceNamedlist.ID, bbytes)
	assert.NilError(t, err)
	itf, err := db.GetCompressingDataDao().GetOne(db.DEFAULT_TENANT_ID, db.TABLE_GENERIC_NS_LIST, sourceNamedlist.ID)
	assert.NilError(t, err)
