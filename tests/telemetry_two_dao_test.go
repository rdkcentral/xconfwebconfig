...
func TestTelemetryTwoDaoSampleData(t *testing.T) {
	...
	for _, x := range mykeys {
		itf, err := db.GetCachedSimpleDao().GetOne(db.GetDefaultTenantId(), db.TABLE_TELEMETRY_TWO_RULES, x)
		assert.NilError(t, err)
		fetchedT2Rule, ok := itf.(*logupload.TelemetryTwoRule)
		assert.Assert(t, ok)
	...
	for profileUuid, profileName := range SampleProfileIdNameMap {
		...
		assert.NilError(t, err)
		err = db.GetCachedSimpleDao().SetOne(db.GetDefaultTenantId(), db.TABLE_TELEMETRY_TWO_PROFILES, profileUuid, &sourceT2Profile)
		assert.NilError(t, err)
		// get a t2profile
		itf, err := db.GetCachedSimpleDao().GetOne(db.GetDefaultTenantId(), db.TABLE_TELEMETRY_TWO_PROFILES, profileUuid)
		assert.NilError(t, err)
